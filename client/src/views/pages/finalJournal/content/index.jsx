...
const HomePage = () => {
  ...
  const deleteJournalHandler = async (journalId) => {
    ...
    try {
      ...
      const newJournalData = [...journalListData].filter(
        (e) => e.id !== journalId
      );
    ...
    const filteredData = journalListData.filter((item) => {
      if (item.category) {
        return item.category.some((category) =>
          category.text.toLowerCase().includes(input)
        );
      }
  ...
  const journalUpdate = async (journalId, journal) => {
    ...
    try {
      ...
      const journalIndex = newJournalListData.findIndex(
